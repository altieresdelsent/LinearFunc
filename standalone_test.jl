# standalone_test.jl — Tests solveSimplified against a reference solve
...
# ---- Include the solveSimplified function ----
include("src/solveSimplified.jl")
...
function runtest(name, f1, f2)
    ...
    r = solveRef(f1, f2)
    s = solveSimplified(f1, f2)
    ok = cmp(r, s)
    if !ok
        println("FAIL: $name")
        println("  ref:  hasSol=$(r.hasSolution) block=$(r.blockVision) many=$(r.hasManySolution) x=$(r.x) y=$(r.y)")
        println("  simp: hasSol=$(s.hasSolution) block=$(s.blockVision) many=$(s.hasManySolution) x=$(s.x) y=$(s.y)")
        global failed += 1
    else
        global passed += 1
    end
